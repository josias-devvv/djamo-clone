import React, { useState } from "react";
import "./style.css";
import Buttony from "../button";
import IonIcon from "@reacticons/ionicons";

export default function Navigation() {
  const [isOpen, setOpen] = useState(false);
  return (
    <div className="nav-container">
      <div id="nav">
        <nav>
          <svg
            width="129"
            height="40"
            viewBox="0 0 129 40"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M19.9444 24.1684H25.2001L21.8879 30.4207H16.03C13.4569 30.4207 12.3483 29.8306 12.3209 28.0884C10.788 29.9149 8.8445 30.8 6.46302 30.8C4.25947 30.8 2.54864 29.8727 1.30315 28.0181C0.0987233 26.1354 -0.270816 23.761 0.19453 20.9088C1.33052 14.1367 5.58708 10.7787 12.9779 10.7787H13.279L12.0472 16.8202H11.6776C9.6657 16.8202 8.29704 18.3657 7.90012 21.1196C7.59902 23.0725 8.40653 24.3511 9.9805 24.3511C11.5818 24.3511 12.4578 23.1427 13.0874 20.0939L16.3037 4.40002H24.0367L19.9444 24.1684Z"
              fill="black"
            ></path>
            <path
              d="M38.0666 0.799988C35.6468 0.799988 33.6859 1.99147 33.2408 4.41748C32.7263 7.08754 34.3673 8.56611 36.9541 8.56611C39.3739 8.56611 41.3766 7.37464 41.8216 4.94863C42.3223 2.26421 40.6534 0.799988 38.0666 0.799988Z"
              fill="black"
            ></path>
            <path
              d="M27.6921 16.9065H31.0575L26.4404 39.2001H34.3674L35.5495 33.7595L37.3852 24.8019L40.3474 10.5185H31.0575L27.6921 16.9065Z"
              fill="black"
            ></path>
            <path
              d="M60.9919 24.2941H65.9998L62.6051 30.5336H56.952C54.3428 30.5336 53.2486 29.9167 53.1504 28.2341C51.8178 29.9447 50.0223 30.8 47.7358 30.8C45.5054 30.8 43.7238 29.8746 42.3491 27.9817C41.0165 26.1028 40.5255 23.8033 40.9464 21.0271C41.3251 18.3209 42.5175 16.0915 44.5094 14.3108C48.4932 10.7073 54.5953 9.64163 62.03 10.9176L63.2644 11.1559L59.8697 16.9889L58.5371 16.8487C53.1504 16.3018 49.6155 17.746 48.9001 20.999C48.381 23.1583 49.1385 24.4623 50.6816 24.4623C52.3228 24.4623 53.2907 23.2004 53.936 20.2138L54.2446 18.7416H62.1703L60.9919 24.2941Z"
              fill="black"
            ></path>
            <path
              d="M99.4752 24.4473H104.4L101.043 30.8H94.7029C93.0382 30.8 91.9561 30.486 91.4428 29.8578C90.9295 29.2297 90.8601 28.0448 91.2347 26.2603L92.5249 19.979C92.9272 17.9233 92.5249 17.1096 91.0266 17.1096C89.5283 17.1096 88.9873 17.7663 88.4463 20.3216L86.2821 30.8H78.4162L80.6497 19.979C81.0937 17.9233 80.6497 17.1096 79.1515 17.1096C78.3746 17.1096 77.8335 17.3523 77.459 17.8091C77.126 18.2659 76.8208 19.2367 76.5156 20.6785L74.4069 30.8H66.541L69.3572 17.181H66L69.3572 10.8283H74.1711C76.7098 10.8283 77.7364 11.385 77.8752 13.0553C79.2347 11.385 81.3017 10.4 83.1607 10.4C86.1434 10.4 87.9052 11.485 88.4463 13.6834C90.4162 11.485 92.622 10.4 95.022 10.4C100.072 10.4 101.736 13.3694 100.419 19.822L99.4752 24.4473Z"
              fill="black"
            ></path>
            <path
              d="M105.928 19.8241C107.432 12.6291 113.166 10.4 118.534 10.4C121.71 10.4 124.197 11.1336 126.207 12.8971C128.217 14.6183 128.835 17.3411 128.104 21.3054C127.373 24.7901 125.589 27.4 123.2 28.8249C120.867 30.1793 118.337 30.8 115.456 30.8C109.484 30.7859 104.214 27.8515 105.928 19.8241ZM119.967 20.1486C120.263 18.3851 119.307 17.2565 117.817 17.2565C115.695 17.2565 114.417 18.4697 114.023 20.8822C113.77 22.6739 114.754 23.8166 116.257 23.8166C118.394 23.8166 119.532 22.7585 119.967 20.1486Z"
              fill="black"
            ></path>
          </svg>
          <ul>
            <li>Accueil</li>
            <li>Produits & Services</li>
            <li>Tarif</li>
            <li>Témoignages</li>
            <li>Points relais</li>
            <li>Apprendre</li>
          </ul>
          <div style={{ display: "flex" }}>
            <div className="iButton">
              <Buttony text="Ouvrir un compte gratuit" top={16} left={32} />
            </div>
            <div
              style={{
                marginLeft: "12px",
                marginRight: "12px",
              }}
              className="menu-but"
              onClick={() => setOpen(!isOpen)}
            >
              <IonIcon name="menu" style={{ fontSize: "35px" }} />
            </div>
          </div>
        </nav>
      </div>
      <div id="menu-drop" className={isOpen ? "open" : "close"}>
        <ul>
          <li>Accueil</li>
          <li>Produits & Services</li>
          <li>Tarif</li>
          <li>Témoignages</li>
          <li>Points relais</li>
          <li>Apprendre</li>
        </ul>
      </div>
    </div>
  );
}
